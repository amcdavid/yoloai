...
- `Dockerfile` — optional. Used with Docker and Podman backends to build a `yoloai-cli-<profile>` image. Uses `FROM yoloai-base` by default; when `base:` is set it must be **FROM-less** (yoloAI supplies the `FROM`). Ignored with Tart and Seatbelt backends. When absent (and no `base:`), Docker/Podman backends use `yoloai-base`.
...
# isolation: container-privileged         # Docker/Podman (--privileged, all caps; use for Docker-in-Docker); needs os=linux (runs via the backend's Linux VM on macOS too); not available with os=mac
...
| `base`                 | Profile overrides baked-in (last non-empty wins). Docker/Podman only; usage error on Tart/Seatbelt. |
