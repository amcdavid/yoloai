...
func TestProfileImageNeedsBuild_MarkerIsPerBackend(t *testing.T) {
	...
	require.True(t, ProfileImageNeedsBuild(dir, nil, parent, "docker"), "nothing built yet")
	RecordProfileBuildChecksum(dir, nil, "docker")

	assert.False(t, ProfileImageNeedsBuild(dir, nil, parent, "docker"),
		"docker built it and recorded under its own key, so docker is up to date")
	assert.True(t, ProfileImageNeedsBuild(dir, nil, parent, "podman"),
...
func TestProfileImageNeedsBuild_LegacyUnkeyedMarkerForcesRebuild(t *testing.T) {
	dir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(dir, "Dockerfile"), []byte("FROM yoloai-base"), 0600))
	require.NoError(t, os.WriteFile(filepath.Join(dir, lastBuildPrefix), []byte(profileBuildChecksum(dir, nil)), 0600))

	assert.True(t, ProfileImageNeedsBuild(dir, nil, t.TempDir(), "docker"),
